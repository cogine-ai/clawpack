...
export function registerImportCommand(command: Command): void {
  command
    ...
    .option('--force', 'Overwrite existing workspace files and runtime files in-place (does not remove unrelated files), and update an existing OpenClaw config entry when --config is provided')
